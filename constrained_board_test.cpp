...
TEST(ConstrainedBoardTest, InvalidTrivial) {
  ...
  ASSERT_THAT(
      constrainedBoard.TrivialCells(), UnorderedElementsAre(std::make_pair(&board(1, 1), 0)));
}
...
TEST(ConstrainedBoardTest, TrivialNecessary) {
  ...
  ASSERT_THAT(constrainedBoard.TrivialCells(), IsEmpty());
  // Setting the sum 10 last here covers the case where the trivial cell is contained in the block
  // we set the sum for.
  constrainedBoard.SetBlockSum(board(0, 3), /* isRow */ false, 10, sumUndo);
...
TEST(ConstrainedBoardTest, TrivialNecessary2) {
  Board board{5, 6};
  board.MakeBlock(board(3, 2));
  board.MakeBlock(board(4, 2));
  board.MakeBlock(board(1, 4));
  ...
  constrainedBoard.SetBlockSum(board(0, 3), /* isRow */ false, 10, sumUndo);
  ASSERT_THAT(constrainedBoard.TrivialCells(), IsEmpty());
  // Setting the sum 6 last here covers the case where the trivial cell is not contained in the
  // block we set the sum for, but instead checked by removed number constraints.
  constrainedBoard.SetBlockSum(board(4, 2), /* isRow */ true, 6, sumUndo);
